...
// MATRIZ DE 10X1O INGRESADO POR TECLADO QUE SUME TODAS LA COLUMNAS IMPARES DE LA MATRIZ
    ...
    public static void main(String[] args) {
        ...
        int sumaColumnasImpar = 0;
        for (int j = 0; j < matriz.length; j++) {
            for (j = 0; j < 5; j++) {
                if (j % 2  == 0) {
                    for (i = 0; i < matriz.length; i++) {
                        sumaColumnasImpar += matriz[i][j];
                    }
                    System.out.print("\nLa suma de la columna imapar " + (j + 1) + " es: " + sumaColumnasImpar);
                    sumaColumnasImpar = 0;
                }
                System.out.println("");
            }
